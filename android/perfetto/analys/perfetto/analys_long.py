#!/usr/bin/python3
# -*- coding: utf-8 -*-

'''
pip3 install perfetto
py -3 analys.py -f redmi.compressed-perfetto-trace
'''

import logging
import sys
from getopt import getopt
from pathlib import Path
from pprint import pprint
from protos import perfetto_trace_pb2

def read_varint(stream):
    shift = 0
    result = 0
    while True:
        byte = stream.read(1)
        if not byte:
            return None
        b = byte[0]
        result |= (b & 0x7F) << shift
        if not (b & 0x80):
            break
        shift += 7
    return result

def read_perfetto(filename):
    packets = []
    with open(filename, 'rb') as f:
        offset = 0
        while True:
            size = read_varint(f)
            if size is None:
                break
            data = f.read(size)
            if len(data) != size:
                print(f"[!] Unexpected EOF at offset {offset}")
                break
            try:
                packet = perfetto_trace_pb2.TracePacket()
                packet.ParseFromString(data)
                packets.append(packet)
            except Exception as e:
                print(f"[!] Failed to parse TracePacket at offset {offset}, size {size}: {e}")
                break
            offset += size
    return packets

def traverse_ftrace_event(packets):
    i = 0
    for packet in packets:
        print("------------------------------")
        print("packet:", i)
        i = i + 1
        which_oneof = packet.WhichOneof('data')
        if which_oneof == 'ftrace_events':
            for event in packet.ftrace_events.event:
                which_oneof = event.WhichOneof('event')
                if which_oneof == 'print':
                    pprint(event.print.buf)
        print(packet)

def main():
    opt, filenames = getopt(sys.argv[1:], 'f', ['file'])
    if not filenames:
        logging.error("please input a filename!")
        return
    for filename in filenames:
        logging.debug("input file: " + filename)
        file_path = Path(filename)
        if not file_path.exists():
            logging.error(filename + ": not exist!")
            return
        if not file_path.is_file():
            logging.error(filename + ": not regular file!")
            return
        packets = read_perfetto(filename)
        traverse_ftrace_event(packets)

if __name__ == '__main__':
    main()